...
class VideoCommentTests(TestCase):
    def test_ci_finished_comment_includes_selenoid_link(self) -> None:
        body = ci_finished_comment(
            ...
            conclusion="success",
            video_selenoid_url="https://selenoid.qa.guru/video/abc-123.mp4",
        ...
        self.assertIn("https://selenoid.qa.guru/video/abc-123.mp4", body)
    ...
    def test_find_selenoid_video_url_in_html_attach(self) -> None:
        html = (
            "<video><source src='https://selenoid.qa.guru/video/dead-beef.mp4'"
            " type='video/mp4'></video>"
        )
        self.assertEqual(
            find_selenoid_video_url(html),
            "https://selenoid.qa.guru/video/dead-beef.mp4",
        )
    ...
    def test_scan_tree_for_selenoid_video_url(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            (root / "allure-results" / "Video.html").write_text(
                "<source src='https://selenoid.qa.guru/video/cafe-babe.mp4'>",
                encoding="utf-8",
            )
            self.assertEqual(
                scan_tree_for_selenoid_video_url(root),
                "https://selenoid.qa.guru/video/cafe-babe.mp4",
            )
