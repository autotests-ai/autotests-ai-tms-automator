...
| Config | `tests-java/allurerc.json` | Plugins awesome + dashboard, categories |

Gradle: `./gradlew allureAnalyticsIndex` · Script: `tests-java/scripts/build-analytics-index.mjs` (`--config allurerc.json` для `charts.failureTaxonomy`)
...
- `qualityGate` в index — те же rules, что `./gradlew allureQualityGate` (`allurerc.json`, `known.json`)

## Do

- Dashboard читает index через HTTP: `../tests-java/build/analytics-index.json` или symlink `frontend/allure-report/analytics-index.json`
