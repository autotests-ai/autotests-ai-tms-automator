...
`layer` / `epic` — из Allure labels (`labelValue`). Epic нужен для будущего linked filter; в harness пока нет epic-chart.
...
Mock-источник: `frontend/analytics-index.mock.json` (14 тестов, sparklines, flaky, taxonomy).
...
- Быстрый mock: `run-live-dashboard-preview.sh --poll 200` (генератор + harness синхронно)
